...
(3) Call Load_DSI_API() in your code before calling any of the other functions.
    ...
    For example, using Visual C++ to compile a 64-bit binary on a Windows
    machine, you could say:

        cl /DDSI_PLATFORM=-Windows-x64  demo.c DSI_API_Loader.c
    ...
    Finally, on 64-bit Linux:
        ...
        gcc -DDSI_PLATFORM=-Linux-x64  demo.c DSI_API_Loader.c -ldl -lm
...
function in all our subsequent demos.
    ...
    int StartUp( int argc, const char * argv[], DSI_Headset * headsetOut, int * helpOut )
    {
        ...
        int          sampleRate = GetIntegerOpt( argc, argv, "samplerate", "f", 0 );
		...
		if( sampleRate )
		{
			DSI_Headset_ConfigureADC( h, sampleRate, 1 ); CHECK
		}
...
before finally de-initializing the DSI_Headset object:
    ...
    int Finish( DSI_Headset h )
    {
        ...
        return 0;
                                                                              ...
                                                                              /*
4.1  Connection robustness and link loss  (new in v1.21.0)

Bluetooth links drop.  As of v1.21.0, every serial write is bounded to 20
seconds via `SetCommTimeouts`, so any API call on a dead link will return
with an error rather than blocking forever.  This is automatic --- you do
not need to change anything to get this behavior.  The tools below let you
detect a dead link earlier and respond to it cleanly.


4.1.1  DSI_Headset_IsConnected()

Returns non-zero if the headset is alive, zero if it is not.  While data is
actively streaming, this returns immediately (the data itself is evidence of
life).  After 2 seconds of silence it sends a bounded Echo probe (~5 seconds
worst-case) before returning.  Use this if your application already has a
reconnection loop:

    if( !DSI_Headset_IsConnected( h ) )
    {
        DSI_Headset_Delete( h );
        h = DSI_Headset_New( NULL );
        DSI_Headset_Connect( h, port );
    }

`DSI_Headset_Delete` is safe to call on a dead link: internal cleanup
exceptions are caught and reported through the message callback rather than
crashing the process.


4.1.2  DSI_Headset_SetWatchdogInterval()

For applications running `DSI_Headset_Idle( h, 0 )` indefinitely --- where
no commands are being sent and `SetCommTimeouts` therefore never fires ---
the watchdog provides passive link monitoring.  It runs on a background
thread and sends a bounded Echo probe after the specified number of seconds
of silence.  If the probe fails, it pushes a level-1 message to your
registered `DSI_MessageCallback`:

    // Recommended minimum: 30 seconds.  The headset has ~15 seconds of
    // onboard memory, so probing too soon may interrupt data still in transit.
    DSI_Headset_SetWatchdogInterval( h, 30.0 );  // 0.0 = off (default)

The message arrives on the watchdog's background thread, not on your call
stack.  Your callback should only set a flag; your main thread does the rest.

Here is an example of how to wire up the watchdog to detect link loss.
The callback watches for the `[lvl1] DSI link lost` notification and sets
a flag when it arrives --- your main thread checks the flag and decides
what to do:

    static volatile int g_link_lost = 0;

    int MyMessage( const char * msg, int level )
    {
        fprintf( stderr, "[lvl%d] %s\n", level, msg );
        if( level == 1 && strstr( msg, "link lost" ) ) g_link_lost = 1;
        return 0;  // do NOT call DSI_Headset_Delete from inside the callback
    }

What you do once the flag is set is entirely up to you.  Here is one
example --- deleting the stale handle and reconnecting automatically:
...
as shown in the `buffering` demo below:
    ...
    int BufferingDemo( int argc, const char * argv[] )
    {
        ...
        while( DSI_Headset_SecondsSinceConnection( h ) < durationSec )
        {
            ...
            printf( "%9.4f,%9.4f,%3zu,%3zu,%3d,%3d\n",
